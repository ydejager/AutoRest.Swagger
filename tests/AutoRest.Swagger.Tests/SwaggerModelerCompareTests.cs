...
namespace AutoRest.Swagger.Tests
{
    ...
    public class SwaggerModelerCompareTests
    {
        ...
        /// </summary>
        /// <param name="input">
        ///     The name of the swagger document file. The file name must be the same in both the 'modified' and
        ///     'original' folder.
        /// </param>
        ...
        private IEnumerable<ComparisonMessage> CompareSwagger(string input)
        {
            ...
            var baseDir = Directory.GetParent(typeof(SwaggerModelerCompareTests).GetTypeInfo().Assembly.Location)
                .ToString();
        ...
        public void AddedPaths()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.AddedPath.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(
                missing.Where(m => m.Severity == Category.Info && m.Path.ReadablePath == "#/paths/api/Paths"));

            missing = messages.Where(m => m.Id == ComparisonMessages.AddedOperation.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Info && m.Path.ReadablePath == "#/paths/api/Operations/post"));
        }
        ...
        public void AddedPropertyInResponse()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.AddedPropertyInResponse.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(missing.Where(m => m.Severity == Category.Error && m.Path.ReadablePath ==
                                               "#/paths/subscriptions/{subscriptionId}/providers/Microsoft.Storage/checkNameAvailability/post/200/properties"));
        }
        ...
        public void AddedRequiredProperty()
        {
            ...
            var missing = messages.Where(m =>
                m.Severity == Category.Error && m.Id == ComparisonMessages.AddedRequiredProperty.Id).ToArray();
        ...
        public void DefinitionRemoved()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.RemovedDefinition.Id).ToArray();
        ...
        public void GlobalResponseTypeConstraintsChanged()
        {
            var messages = CompareSwagger("response_check_01.json")
                .Where(m => m.Path.JsonReference.Contains("responses")).ToArray();
            var relaxed = messages
                .Where(m => m.Id == ComparisonMessages.ConstraintIsWeaker.Id && m.Severity == Category.Error).ToArray();
            var breaking = messages
                .Where(m => m.Id == ComparisonMessages.ConstraintChanged.Id && m.Severity == Category.Error).ToArray();
        ...
        {
            var messages = CompareSwagger("param_check_01.json").Where(m => m.Path.JsonReference.Contains("parameters"))
                .ToArray();
            var stricter = messages
                .Where(m => m.Id == ComparisonMessages.ConstraintIsStronger.Id && m.Severity == Category.Error)
                .ToArray();
            var breaking = messages
                .Where(m => m.Id == ComparisonMessages.ConstraintChanged.Id && m.Severity == Category.Error).ToArray();
            var info = messages.Where(m => m.Id > 0 && m.Severity == Category.Info).ToArray();
        ...
        public void OperationIdChanged()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.ModifiedOperationId.Id).ToArray();
            Assert.Equal(2, missing.Length);
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Paths/get"));
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Operations/post"));
        }
        ...
        public void OperationRemoved()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.RemovedOperation.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Operations"));
        }
        ...
        public void ParameterConstantChanged()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.ConstantStatusHasChanged.Id).ToArray();
            Assert.NotEmpty(missing);
            var error = missing.First();
        ...
        {
            var messages = CompareSwagger("operation_check_01.json").ToArray();
            var missing = messages.Where(m => m.Id == ComparisonMessages.ParameterInHasChanged.Id).ToArray();
            Assert.NotEmpty(missing);
            var error = missing.First();
            Assert.Equal(Category.Error, error.Severity);
        ...
        public void ParameterSchemaContentsChanged()
        {
            ...
            var changed = messages.Where(m => m.Id == ComparisonMessages.TypeChanged.Id).ToArray();
        ...
        public void ParameterStatusLess()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.RequiredStatusChange.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Parameters/{a}/get/e"));
        }
        ...
        public void PathRemoved()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.RemovedPath.Id).ToArray();
            Assert.Equal(2, missing.Length);
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Parameters/{a}"));
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Responses"));
        }
        ...
        public void PropertyTypeChanged()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.TypeChanged.Id).ToArray();
            Assert.NotEmpty(missing);
            var error = missing.FirstOrDefault(err => err.Path.JsonReference.StartsWith("#/definitions/"));
        ...
        public void PropertyTypeFormatChanged()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.TypeFormatChanged.Id).ToArray();
            Assert.NotEmpty(missing);
            var error = missing.FirstOrDefault(err => err.Path.JsonReference.StartsWith("#/definitions/"));
            Assert.NotNull(error);
            Assert.Equal(Category.Error, error.Severity);
        ...
        public void ReadonlyPropertyInResponse()
        {
            ...
            var missing = messages.Where(m => m.Id == ComparisonMessages.AddedReadOnlyPropertyInResponse.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(missing.Where(m => m.Severity == Category.Info && m.Path.ReadablePath ==
                                               "#/paths/subscriptions/{subscriptionId}/providers/Microsoft.Storage/checkNameAvailability/post/200/properties"));
        }
        ...
        public void RemovedPropertyTest()
        {
            ...
            Assert.Contains(messages, m => m.Id == ComparisonMessages.RemovedProperty.Id);
        }
        ...
        public void RequestArrayFormatChanged()
        {
            var messages = CompareSwagger("operation_check_04.json")
                .Where(m => m.Path.JsonReference.Contains("Parameters")).ToArray();
            var changed = messages.Where(m => m.Id == ComparisonMessages.ArrayCollectionFormatChanged.Id).ToArray();
        ...
        {
            var messages = CompareSwagger("misc_checks_01.json").ToArray();
            var missing = messages.Where(m => m.Id == ComparisonMessages.RequestBodyFormatNoLongerSupported.Id).ToArray();
            Assert.NotEmpty(missing);
            Assert.Equal(Category.Error, missing.First().Severity);
        }
        ...
        {
            var messages = CompareSwagger("required_parameter.json").ToArray();
            var missing = messages.Where(m => m.Id == ComparisonMessages.AddingRequiredParameter.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Parameters/{a}/get/g"));
        }
        ...
        {
            var messages = CompareSwagger("required_parameter.json").ToArray();
            var missing = messages.Where(m => m.Id == ComparisonMessages.RemovedRequiredParameter.Id).ToArray();
            Assert.Single(missing);
            Assert.NotEmpty(missing.Where(m =>
                m.Severity == Category.Error && m.Path.ReadablePath == "#/paths/api/Parameters/{a}/get/f"));
        }
        ...
        {
            var messages = CompareSwagger("operation_check_01.json").ToArray();
            var removed = messages.Where(m => m.Id == ComparisonMessages.AddingResponseCode.Id).ToArray();
        ...
        {
            var messages = CompareSwagger("misc_checks_01.json").ToArray();
            var added = messages.Where(m => m.Id == ComparisonMessages.ResponseBodyFormatNowSupported.Id).ToArray();
        ...
        {
            var messages = CompareSwagger("operation_check_03.json").ToArray();
            var changed = messages.Where(m =>
                m.Id == ComparisonMessages.TypeChanged.Id && m.Path.JsonReference.Contains("Responses")).ToArray();
        ...
        {
            var messages = CompareSwagger("operation_check_01.json").ToArray();
            var removed = messages.Where(m => m.Id == ComparisonMessages.RemovedResponseCode.Id).ToArray();
            Assert.NotEmpty(removed);
            var error = removed.First();
            Assert.Equal(Category.Error, error.Severity);
        ...
        public void ReversedVersionNumberChange()
        {
            ...
            var reversed = messages.Where(m => m.Id == ComparisonMessages.VersionsReversed.Id).ToArray();
        ...
        public void UnreferencedDefinitionRemoved()
        {
            ...
            var missing =
                messages.Where(m => m.Id > 0 && m.Path.JsonReference.StartsWith("#/definitions/Unreferenced"));
