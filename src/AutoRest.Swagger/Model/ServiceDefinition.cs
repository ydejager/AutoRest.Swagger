...
namespace AutoRest.Swagger.Model
{
    ...
    public class ServiceDefinition : SpecObject
    {
        ...
        public override IEnumerable<ComparisonMessage> Compare(ComparisonContext context, SwaggerBase previous)
        {
            ...
            if (context.Strict) context.LogInfo(ComparisonMessages.NoVersionChange);
                ...
                else
                {
                    ...
                    foreach (var previousOperation in previousOperations)
                    {
                        ...
                        if (!operations.TryGetValue(previousOperation.Key, out newOperation))
                            context.LogBreakingChange(ComparisonMessages.RemovedOperation,
                                previousOperation.Value.OperationId);
                        else
            ...
            foreach (var path in previousDefinition.CustomPaths.Keys)
            {
                ...
                else
                {
                    ...
                    // 2. look for operation match inside this path
                    var previousOperations = previousDefinition.CustomPaths[path];
                    foreach (var previousOperation in previousOperations)
                    {
                        Operation newOperation = null;
                        if (!operations.TryGetValue(previousOperation.Key, out newOperation))
            ...
            {
                Schema schema = null;
                var oldSchema = previousDefinition.Definitions[def];
        ...
        private void CompareVersions(ComparisonContext context, string newVer, string oldVer)
        {
            ...
            if (!context.Strict && oldVersion.Length > 0 && newVersion.Length > 0)
            {
                ...
                var integers = int.TryParse(oldVersion[0], out oldMajor) && int.TryParse(newVersion[0], out newMajor);

                if (integers && oldMajor != newMajor)
                {
                    versionChanged = true;

                    if (oldMajor > newMajor) context.LogError(ComparisonMessages.VersionsReversed, oldVer, newVer);
                }
                ...
                if (!versionChanged && integers && oldVersion.Length > 1 && newVersion.Length > 1)
                {
                    ...
                    if (integers && oldMinor != newMinor)
                    {
                        ...
                        if (oldMinor > newMinor) context.LogError(ComparisonMessages.VersionsReversed, oldVer, newVer);
                    }
                }
                ...
                if (!versionChanged && !integers) versionChanged = !oldVer.ToLower().Equals(newVer.ToLower());
        ...
        {
            foreach (var schema in service.Definitions.Values) schema.IsReferenced = false;
