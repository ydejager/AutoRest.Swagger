...
namespace AutoRest.Swagger.Model
{
    ...
    public class Schema : SwaggerObject
    {
        ...
        private readonly LinkedList<Schema> _visitedSchemas = new LinkedList<Schema>();
        ...
        [JsonIgnore] internal bool IsReferenced { get; set; }
        ...
        {
            if (!(previous is Schema priorSchema)) throw new ArgumentNullException("priorVersion");
            ...
            {
                thisSchema = FindReferencedSchema(thisSchema.Reference,
                    (context.CurrentRoot as ServiceDefinition).Definitions);
                referenced += 1;
                if (thisSchema == null) return context.Messages;
            }

            if (!string.IsNullOrWhiteSpace(priorSchema.Reference))
            {
                priorSchema = FindReferencedSchema(priorSchema.Reference,
                    (context.PreviousRoot as ServiceDefinition).Definitions);
                referenced += 1;
                if (priorSchema == null) return context.Messages;
            }
            ...
            if (context.Direction != DataDirection.None && referenced == 2)
            {
                ...
                if (thisSchema._compareDirection == context.Direction ||
                    thisSchema._compareDirection == DataDirection.Both) return new ComparisonMessage[0];
                _compareDirection |= context.Direction;
            }

            if (thisSchema != this || priorSchema != previous)
            {
                if (_visitedSchemas.Contains(priorSchema)) return context.Messages;
            ...
            if (priorSchema.ReadOnly != ReadOnly)
                context.LogBreakingChange(ComparisonMessages.ReadonlyPropertyChanged,
                    priorSchema.ReadOnly.ToString().ToLower(), ReadOnly.ToString().ToLower());
            ...
            else if (priorSchema.AllOf != null) CompareAllOfs(context, priorSchema);
        ...
        private void CompareRequired(ComparisonContext context, Schema priorSchema)
        {
            ...
            var addedRequiredProperties = Required.Except(priorSchema.Required).ToList();
        ...
        private void CompareAllOfs(ComparisonContext context, Schema priorSchema)
        {
            ...
            if (different > 0) context.LogBreakingChange(ComparisonMessages.DifferentAllOf);
        }
        ...
        public static Schema FindReferencedSchema(string reference, IDictionary<string, Schema> definitions)
        {
            if (reference != null && reference.StartsWith("#", StringComparison.Ordinal))
            {
                ...
                {
                    Schema p = null;
                    if (definitions.TryGetValue(parts[2], out p)) return p;
                }
            }
