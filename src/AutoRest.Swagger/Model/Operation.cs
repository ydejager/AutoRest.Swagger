...
namespace AutoRest.Swagger.Model
{
    ...
    public class Operation : SwaggerBase
    {
        ...
        public override IEnumerable<ComparisonMessage> Compare(ComparisonContext context, SwaggerBase previous)
        {
            ...
            if (priorOperation == null) throw new ArgumentException("previous");
        ...
        private void CheckParameters(ComparisonContext context, Operation priorOperation)
        {
            ...
            {
                var newParam = FindParameter(oldParam.Name, Parameters, currentRoot.Parameters);
            ...
            foreach (var newParam in requiredParamters)
            {
                ...
                var oldParam = FindParameter(newParam.Name, priorOperation.Parameters, previousRoot.Parameters);
        ...
        private SwaggerParameter FindParameter(string name, IEnumerable<SwaggerParameter> operationParameters,
            IDictionary<string, SwaggerParameter> clientParameters)
        {
            if (Parameters != null)
                foreach (var param in operationParameters)
                {
                    ...
                    if (pRef != null && name.Equals(pRef.Name)) return pRef;
                }
            ...
            IDictionary<string, SwaggerParameter> parameters)
        {
            if (reference != null && reference.StartsWith("#", StringComparison.Ordinal))
            {
                ...
                if (parts.Length == 3 && parts[1].Equals("parameters"))
                {
                    ...
                    if (parameters.TryGetValue(parts[2], out p)) return p;
                }
            }
